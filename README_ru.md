...
Для работы скрипта необходим **Node.js** (версия 22 или выше) - это программная платформа, которая позволяет запускать JavaScript код на вашем компьютере.
...
4. После установки проверьте, что всё работает, открыв терминал/командную строку и введя:
   ```bash
   node --version
   ```
   Вы должны увидеть номер версии вида `v22.x.x` или выше

**Примечание:** Если вы видите версию ниже 22, вам нужно обновить Node.js до более новой версии.
